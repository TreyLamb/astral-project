...
with open("cpMultiplier.json", encoding="utf-8") as f:
    cpm_raw = json.load(f)
CPM = {entry["level"]: entry["multiplier"] for entry in cpm_raw}

with open("pokemonStats.json", encoding="utf-8") as f:
    stats = json.load(f)

with open("pokemonEvolutions.json", encoding="utf-8") as f:
...
with open("cpTable.txt", "w", newline="", encoding="utf-8") as f:
    for section in SECTION_ORDER:
...
print(f"Wrote cpTable.txt across {len(SECTION_ORDER)} sections")
