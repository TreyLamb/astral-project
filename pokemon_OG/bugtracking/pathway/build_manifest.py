...
checking only ONE data source (e.g. game_data.json's flat warps/npcs
...
and adds two things audit_map.py doesn't do:
  1. A three-way filename reconciliation across scripts/*.asm,
     data/maps/objects/*.asm, and game_data.json's map keys (unreferencedOgFiles).
...
# ROOT = astral-project/pokemon_OG/bugtracking/pathway
...
AUDIT_SCRIPT_DIR = os.path.join(REPO, "pokemon_OG", "bugtracking", "Lastmap markdowns")
...
maps = audit_map.maps  # dict MAP_ID -> map info, straight from game_data.json
...
# game_data.json entry yet (e.g. UNDERGROUND_PATH_ROUTE_7, not yet converted).
...
def resolve_map_id(key):
    ...
    known (via game_data.json directly, or via map_constants.asm for
...
for key in sorted(union_keys):
    ...
    (present_in if in_game_data else missing_from).append("game_data.json")
...
#
# Built by cross-referencing game_data.json's own `connections`/`warps`
...
# union (221 game_data.json maps + UNDERGROUND_PATH_ROUTE_7 +
# UNDERGROUND_PATH_ROUTE_7_COPY, both real map_const entries missing from
# game_data.json) before this script was finalized.
#
