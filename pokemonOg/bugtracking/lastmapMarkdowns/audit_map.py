...
current JS (gameData.json + PokeredOverworld.jsx + pokeredGameState.js), and
...
-----
  ...
  python3 audit_map.py --all          # every map in gameData.json
...
# ROOT = astral-project/pokemonOg/bugtracking/lastmapMarkdowns
...
item_locations = json.load(open(os.path.join(PAGE_ROOT, "extractedOgData", "itemLocations.json")))
hidden_items   = json.load(open(os.path.join(PAGE_ROOT, "extractedOgData", "hiddenItems.json")))
marts_json     = json.load(open(os.path.join(PAGE_ROOT, "extractedOgData", "marts.json")))
npc_dialogue_json = json.load(open(os.path.join(PAGE_ROOT, "extractedOgData", "npcDialogue.json")))
...
# Both use the exact same "MAP_ID:N" key shape as npcDialogue.json's own per-map indexing,
...
# constant) — PokeredApp.jsx imports the whole of extractedOgData/marts.json wholesale as
...
def audit_connections(map_id):
    ...
    for direction, conn in mi["connections"].items():
        ...
        if not dest:
            record(map_id, f"connections/{direction}", FAIL, f"connects to {dest_id}, which doesn't exist in gameData.json")
            continue
...
def audit_script_complexity(map_id):
    ...
    puzzle floors) that has NO structural analog in gameData.json's flat warps/npcs/
    ...
    what to go verify by hand instead of silently assuming gameData.json's flat data is
    ...
    if badge_checks:
        ...
        record(map_id, "script:complexity", WARN,
               ...
               f"gameData.json's structural counts (Phase 1 checks above) cover it")
...
def audit_map(map_id):
    if map_id not in maps:
        record(map_id, "existence", FAIL, f"{map_id} not found in gameData.json at all")
        return
    mi = maps[map_id]
    pascal = to_pascal(map_id)
    ...
    if obj_asm is None:
        ...
        for label, og_n, js_n in [
            ...
            if og_n == js_n:
                ...
                record(map_id, label, FAIL, f"OG has {og_n}, gameData.json has {js_n} — {'missing' if js_n < og_n else 'extra/fabricated'} entries")
    ...
    if hidden_block_match:
        ...
        if not hidden_lines:
            ...
            for (hx, hy, handler, arg) in hidden_lines:
                # Cross-check against hiddenItems.json coordinates for HiddenItems handlers
                if handler == "HiddenItems":
                    ...
                    if not match:
                        record(map_id, "hidden_events/HiddenItems", FAIL,
                               f"OG hidden_event at ({hx},{hy}) gives {arg}, but hiddenItems.json has no matching entry for {map_id}")
                    else:
                        m = match[0]
                        if str(m.get("x")) != hx or str(m.get("y")) != hy:
                            record(map_id, "hidden_events/HiddenItems coords", FAIL,
                                   f"OG coords ({hx},{hy}) vs hiddenItems.json coords ({m.get('x')},{m.get('y')}) for {arg} — "
    ...
    # ── 3. itemLocations.json presence/wiring (ground items) ──────────
    if map_id in item_locations and item_locations[map_id]:
        ...
        record(map_id, "item_locations", WARN,
               f"{len(item_locations[map_id])} ground item(s) exist in itemLocations.json for {map_id} — "
    ...
    #   1. npcDialogue.json[map][index].text — real per-map OG text (non-scripted entries)
        ...
        if is_scripted_unhandled:
            if sprite not in npc_text_keys:
                record(map_id, "npc_dialogue", FAIL,
                       f"NPC sprite '{sprite}' at ({npc['x']},{npc['y']}) is npcDialogue.json label "
    ...
    if has_clerk and map_id not in marts_json:
        record(map_id, "mart_wiring", FAIL, f"clerk NPC present but '{map_id}' has no entry in extractedOgData/marts.json at all")
