...
pokemonOg into the port until it's 100%" effort at full context — no re-discovery, no
...
1. `src/pages/pokeredPage/CLAUDE.md` — pokered rules, verified architecture facts, gotchas.
   (Auto-loads when cwd is the game folder. Also registers two skills.)
2. `pokeredChecklist.md` (this folder) — live implementation status. Check before assuming
...
**OG source of truth:** `pokemonOg/PokeRed_OG/` (read-only). A map's behavior spans up to 5
...
- **0a Event-flag system** — 507 OG flags → `extractedOgData/eventFlags.json`; `EVENT_FLAGS`
...
- **0c Battle audit** — flinch, confusion (exact 40-power), high-crit, Focus Energy were ALREADY
  ...
  comfortable unless MISSING"). Refinements parked in `battleMechanicsChangeProposals.md`
  (Proposals 1-4) — DO NOT apply without owner approval. `growthRates.json` extracted but unwired.
...
- **Pokémon Tower chain (Agent 2)** ✅ in main: Silph Scope ghost-reveal (`ghostDisguise:
  ...
  (`EVENT_GOT_POKE_FLUTE`), **Snorlax** on R12/R16 (NPCs added to gameData.json, flute-wake → Lv30
...
`src/pages/pokeredPage/CLAUDE.md` (auto-loads); `promptfreeclaude.md` folded in.
...
- **Convert `UNDERGROUND_PATH_ROUTE_7`** (Route 7 entrance building) into gameData.json — Route 7's
...
- **Route 3 / Mt Moon / Route 4 / Cerulean / R24-25** — deep DIALOGUE + ITEM audit still owed
  ...
  (npcDialogue.json done through Cerulean), but verify each vs OG + hidden items.
...
- `battleMechanicsChangeProposals.md` Proposals 1-4 (base-speed crit; per-species XP curves via
  `growthRates.json`; Toxic escalation; ghost-battle "scared/get out" flavor text). Review with owner.
...
3. After each region: build (`npx vite build`), sync `pokeredChecklist.md`, commit, update this
   handoff doc's §2/§3.
