...
- [x] Exclamation mark bubble on trainer LOS trigger (2026-08-02, Phase 4) — found the real,
      ...
      (`pokemonOg/PokeRed_OG/gfx/emotes/shock.png`, confirmed via `EmotionBubble`/
...
- [x] Critical hit — Gen 1 speed-based formula
  ...
  - ⚠️ Uses live Speed stat, not base species Speed — see battleMechanicsChangeProposals.md (Proposal 1, refinement not a bug)
...
- [x] Per-species base XP yield
  - ⚠️ XP growth curve is Medium-Slow for all species — Gen 1 has 4 distinct growth rates.
    Data extracted (`extractedOgData/growthRates.json`, 151/151); wiring is a proposal
    pending approval — see battleMechanicsChangeProposals.md (Proposal 2, visible balance change)
...
- [x] Gym puzzles — Surge trash cans (Vermilion, prior pass). Sabrina's teleport-tile puzzle
      ...
      correctly present in `gameData.json` — `handleWarp` is fully generic for same-map warps,
...
- [x] R9_10-RockTunnel-Lavender-PokemonTower cluster fully wired (22 maps: LAVENDER_CUBONE_HOUSE,
  ...
  - **Power Plant disguised wild Pokémon** (`POWER_PLANT_WILD_OBJECTS`, `PokeredOverworld.jsx`):
    ...
    completely unreachable — `gameData.json`'s extractor collapsed OG's `object_event ... SPECIES,
  ...
  - **Missing story-flag dialogue branches** (found via manual script trace, not caught by the
    structural auditor since these NPCs' `npcDialogue.json` entries were non-`scripted` static
...
- [x] Route3-MtMoon-Cerulean-R24_25-Bill cluster fully wired (17 maps: BIKE_SHOP, BILLS_HOUSE,
  ...
  - **CERULEAN_CITY fabricated NPC removed**: `gameData.json` had 12 npcs vs OG's real 11 — a
    spurious `poke_ball` entry at (28,12), sitting on the exact same tile as the real Guard1
    NPC, backed by an equally-spurious `itemLocations.json` HM06 entry with no OG object_event
...
- [x] R7-Celadon-RocketHideout-Erika cluster fully wired (30 maps: CELADON_CHIEF_HOUSE,
  ...
  - **UNDERGROUND_PATH_ROUTE_7 + `_COPY` converted from scratch** — did not exist in
    `gameData.json` at all. Added as new top-level map entries (tileset `gate`, 4×4, mirrors
    `UNDERGROUND_PATH_ROUTE_5`'s structure exactly), `.blk` copied from
    `pokemonOg/PokeRed_OG/maps/UndergroundPathRoute7.blk` (both maps share it — no separate
  ...
  - **Game Corner slot machines + hidden coins wired** — 33 real `StartSlotMachine` seats (+3
    ...
    entirely unwired* free-coin gifts from fishing_guru/clerk2/gentleman — their `npcDialogue.json`
    ...
    session) — added his real before/win/after lines to `trainerText.json`.
...
- [x] Saffron-SilphCo-Dojo cluster fully wired (21 maps: COPYCATS_HOUSE_1F/2F, FIGHTING_DOJO,
  ...
  - **Sabrina's teleport-tile gym puzzle** — confirmed NOT a bespoke mechanism at all: OG
    ...
    in `gameData.json` (32 warps total, incl. 2 entrance/exit). `handleWarp` is fully generic for
  ...
  - All ground items (`itemLocations.json`) and hidden items (`hiddenItems.json`, incl. the 3
...
|----------------------------------|----------------------------------------------|--------|
| `pokemonData.json`              | Base stats, learnsets, level evolutions      | ✅ Converted & wired |
...
| `gameData.json`                 | Map layouts, NPC positions, warp data        | ⚠️ Wired; 556/799 warps still at placeholder `dir:0` |
...
| `constants/event_constants.asm`  | Named story event flags                       | 🔄 Registry converted (507 flags → `extractedOgData/eventFlags.json`) + typo-safe `hasEvent/setEvent/clearEvent` API in `pokeredGameState.js`; first real consumer wired 2026-07-20 (Cinnabar Lab fossil revival: `EVENT_GAVE_FOSSIL_TO_LAB`/`EVENT_LAB_STILL_REVIVING_FOSSIL`/`EVENT_LAB_HANDING_OVER_FOSSIL_MON`) — most other story gating still uses ad-hoc booleans |
...
3. ~~HM06 ground item at Cerulean doorway (28,12)~~ — ROOT-CAUSED AND FIXED
   ...
   genuinely fabricated `poke_ball` npc entry (`gameData.json`) plus a matching spurious
   `itemLocations.json` HM06 entry, neither backed by any real OG `object_event`, sitting on
...
4. Page load times reported ~5x slower. **Investigated 2026-08-03, confirmed real, root cause
   ...
   something introduced by or fixable inside `pokeredPage/`. Pokered does contribute to that one
   shared bundle too (`extractedOgData/npcDialogue.json` 100K + `trainerText.json` 108K +
   ...
   `fetch()` the way `gameData.json`/`pokemonData.json` already correctly are), but that's a
   ...
   sitewide, outside `pokeredPage`'s own ownership/CLAUDE.md and outside this session's scope —
...
5. Route 11.5 ↔ Route 12 warps (walking into the roof of gates) — 2 warps affected.
   **Investigated 2026-08-03, no data bug found — flagging for a more specific repro rather than
   ...
   `gameData.json` line by line — coordinates, `warpIdx` pairings, and directions are a 100%
...
6. `#`/Poké character rendering issue. **Root-caused and fixed 2026-08-03.** OG's own
   ...
   carried that raw placeholder straight into `gameData.json` (45 occurrences, bg_event
   sign text), `extractedOgData/npcDialogue.json` (50), and `extractedOgData/
   trainerText.json` (8) — since this port renders plain text (no special tile font), the
...
- **Event flags**: registry now exists (507 → `extractedOgData/eventFlags.json`), `hasEvent/setEvent/clearEvent` typo-guarded; still has ZERO `.jsx` consumers — wiring happens in region phases.