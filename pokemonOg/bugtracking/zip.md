...
$root = "c:\Users\PCT\Projects\astral-project"; $stamp = Get-Date -Format "yyyyMMdd_HHmm"; & "C:\Program Files\WinRAR\WinRAR.exe" a -r -ep1 -afzip -ibck "$root\pokemon_backup_$stamp.zip" "$root\src\pages\pokeredPage" "$root\public\pokered" "$root\pokemonOg"; Write-Host "Saved: pokemon_backup_$stamp.zip"
...
`pokemonOg\PokeRed_OG\.git` folder (34 files, ~23MB of packed git history for
...
pokeredPage\, pokered\, pokemonOg\), this produces a genuine PK-format zip —
...
claude " as long as the 3 parent folders keep their names (pokeredPage, pokered, pokemonOg), the command is always safe. It only cares about those 3 paths, nothing inside them matters.
