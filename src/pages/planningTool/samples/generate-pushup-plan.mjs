...
// verification discipline). Run with: node src/pages/planningTool/samples/generate-pushup-plan.mjs
//
...
// (and training.md's copy of it) previously showed Lift A on both D2 and D11
// of the standard 13-day cycle, which contradicted training-context.md's own
// Rule 1 ("3 lift days per cycle: Lift A, Lift B, Lift C"). Cycle 2's D11 is
// therefore plain Easy here, not "Easy + Lift A" — training.md landed this
// same fix in the same commit as this generator (2026-08-04), so this table
// is in sync as of writing; re-sync this generator if training.md's dated
// tables ever change again.
const DAYS = [
...
// ---------------------------------------------------------------------------
// 1b. TWO OPEN RULINGS, MADE CONFIGURABLE — the user's spec doesn't say
// whether the full-rest day or speed days are pushup-eligible; per the task's
// own instruction ("make a defensible call, flag it, and make it
// configurable") these are real named flags, not just a paragraph of prose —
// flip either one and re-run `node generate-pushup-plan.mjs` to get the
// alternate workbook, no code archaeology required.
const EXCLUDE_FULL_REST_DAY = true; // ✂️ ruling: cycle.md/training-context.md both call full rest "no running, no lifting, no exceptions" — a pushup set is still training, so it's treated the same. Flip to `false` to make D13/D12(Cycle1) eligible too.
const EXCLUDE_SPEED_DAYS = false; // ✂️ ruling: no rule in the user's spec touches speed days, and there's no stated physiological conflict the way there is with lifting, so they stay eligible by default. Flip to `true` to exclude them.
...
//  - Full true rest: see EXCLUDE_FULL_REST_DAY above.
//  - Speed days: see EXCLUDE_SPEED_DAYS above.
...
function computeEligibility(days) {
  ...
  // Full-rest / speed-day exclusions applied last so they always win as the
  // stated reason even on a day that's never independently caught by the
  // bench/back windows in this particular schedule.
  if (EXCLUDE_FULL_REST_DAY) {
    days.forEach((d, i) => { if (d.type === 'rest') excluded.set(i, 'full true rest day — no exceptions (EXCLUDE_FULL_REST_DAY=true)'); });
  }
  if (EXCLUDE_SPEED_DAYS) {
    days.forEach((d, i) => { if (d.type === 'speed') excluded.set(i, 'speed day (EXCLUDE_SPEED_DAYS=true)'); });
  }
...
function readmeSheet() {
  const lines = [
    ...
    ['Lift A correction (already applied)', 'training.md and this workbook both now reflect the user\'s 2026-08-04 ruling: ONE Lift A (bench) per cycle, not two — training-context.md\'s own Rule 1 says 3 lift days per cycle (one Lift A, one Lift B, one Lift C). Cycle 2 Day 11 is plain Easy in both. Re-generate this workbook (node generate-pushup-plan.mjs) if training.md\'s dated tables ever change again.'],
    [''],
    ...
    ['No pushups on the full true rest day (CONFIGURABLE)', 'Ruling, on by default: cycle.md/training-context.md both describe full rest as "no running, no lifting, no exceptions" — a pushup set is still training, so it\'s excluded too. Not explicitly stated by the user for pushups. Controlled by the `EXCLUDE_FULL_REST_DAY` constant at the top of generate-pushup-plan.mjs (currently true) — flip it to false and re-run the generator to make D13/Cycle-1-D12 eligible instead.'],
    ['Speed days ARE eligible for pushups (CONFIGURABLE)', 'Ruling, on by default: no rule in the user\'s spec touches speed days, and there\'s no stated conflict the way there is with lifting. Controlled by the `EXCLUDE_SPEED_DAYS` constant (currently false) — flip it to true and re-run the generator to exclude them instead.'],
...
async function styleWorkbook(buffer) {
  ...
  async function styleSheet(sheetPath, dataRows) {
    ...
    //
    // BUG FIXED (found during real-Excel-COM verification, 2026-08-04): SheetJS's
    // own pass-1 output already writes a default `<sheetViews><sheetView
    // workbookViewId="0"/></sheetViews>` for every worksheet. The original version
    // of this code checked `if (!sheetViewsEl)` (correctly finding it already
    // existed) but then unconditionally created and appended a SECOND
    // `<sheetView workbookViewId="0">` anyway, leaving two sibling `<sheetView>`
    // elements with the same workbookViewId. Real Excel (via COM,
    // `Workbooks.Open`) hard-refused the file outright — "Unable to get the Open
    // property of the Workbooks class", no repair prompt, matching the exact
    // symptom documented in PROGRESS.md phase 16's calcChain bug (a different
    // root cause, same class of "structurally invalid enough that Excel won't
    // even attempt repair"). LibreOffice and openpyxl both opened the malformed
    // file without complaint, which is exactly why PROGRESS.md's verification
    // discipline insists on real Excel COM as ground truth, not just those two.
    // Fixed by reusing the existing `<sheetView>` (if SheetJS already wrote one)
    ...
    let sheetViewEl = sheetViewsEl.getElementsByTagName('sheetView')[0];
    if (!sheetViewEl) {
      sheetViewEl = doc.createElementNS(NS_MAIN, 'sheetView');
