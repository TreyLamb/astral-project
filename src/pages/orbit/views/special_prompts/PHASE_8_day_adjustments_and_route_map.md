...
This is **doc §17 Phase 5** of `src/pages/orbit/Guidelines_Scheduler.md` (the "spec"). The deep sections are **§13 (Reschedule / Add X / Remove X, deferability score)** and **§14 (route map)**.
...
- **Do NOT touch** the Firestore coalescing machinery in `orbitContext.js`. **Do NOT** update the external project-memory file. **DO** update the "Where it stands" block in `Guidelines_Scheduler.md` when done.
