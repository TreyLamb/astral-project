...
import ITEM_LOCATIONS from './extractedOgData/itemLocations.json';
import HIDDEN_ITEMS from './extractedOgData/hiddenItems.json';
import NPC_DIALOGUE from './extractedOgData/npcDialogue.json';
import TRAINER_TEXT from './extractedOgData/trainerText.json';
import DEX from './extractedOgData/dex.json';
...
// species-key convention (used by pokemonData.json/party mons/sprites) — aliased here so
...
// destination map's OWN warp_events index (1-indexed, matches gameData.json's warps array
...
// LOGICAL coordinate unit (p.x/p.y, NPC x/y, gameData.json warps/npcs/bgEvents) is now 1 metatile
...
// ============================================================================
// WARP_DIR CONVENTION — gameData.json warp entries use a numeric `dir` field:
...
// in gameData.json, so that file has no matching text to search for.
// ============================================================================
...
export default function PokeredOverworld({ initialMapId, initialX, initialY, onEncounter, onTrainerBattle,speedMult, setSpeedMult, showWarps, setShowWarps, onReturnHome, onHealParty, onPoisonTick, onMarkGiftTaken, onDeliverParcel, onRequestStarter, onOpenPC, onOpenShop, onOpenSlots, onMapChange, onSave, onSaveExtraAsNew, onPositionUpdate, onConsumePendingTrainerPos, onPickUpItem, onUseItem, onTeachMove, onSwitchParty, onSwapMoves, onRenameMon, onBuyMagikarp, onBuyItem, onGiveGuardDrink, onExchangeBikeVoucher, onGiveDollForTM31, onCutTree, onSetSurfing, onActivateStrength, onPushBoulder, onActivateFlash, onMetOldMan, onSetEvent, onToggleEvent, onGiveFossil, onCollectFossilMon, onDoTrade, onGymTrashCan, onDaycareDeposit, onDaycarePay, onDaycareStep, onFindHiddenCoins, onBuyCoins, onGiveDrinkForTM, onBuyPrize, onGivePokemon, onSafariStep, onSafariEnter, onSafariLeave, onGiveHmSurf, onGiveHmStrength, onGiveHmFly, onCompleteHallOfFame, gameState, isExtra }) {
  ...
  // / EVENT_BEAT_ZAPDOS flag (all 9 already present in eventFlags.json, mirrored 1:1 from OG's
  ...
  // trainer-shaped. Confirmed real bug this fixes: gameData.json's extractor collapsed these
  ...
  // when position isn't in itemLocations.json) — Zapdos and all 8 Voltorb/Electrode encounters
  ...
  function isWalkable(tx, ty) {
    ...
    // static block-based gameData.json can't represent directly (VermilionGym.blk's own baked
    ...
    // waterDecorativeTilesByTileset (see its own comment at the gameData.json load site): a
  ...
  // (55/54/39/13/29 in our gameData.json) is confirmed absent from the overworld passable list.
  ...
  // in gameData.json — they only ever move via the chase-to-battle walk-up, never patrol) live
  ...
  // caller (patrol movement for non-Swimmer NPCs, which per gameData.json never patrol on
...
const OUTDOOR = ['overworld', 'plateau'];
    ...
    try {
      ...
      // in gameData.json ((9,11)/(12,9)/(9,7)), so rather than guess at a mismatched table,
      ...
      // port's HALL_OF_FAME npc data has Oak already standing in place (see gameData.json), so
      // the walk-in itself is skipped (same player-forced-movement tradeoff as CHAMPIONS_ROOM)
      // and the real text (npcDialogue.json HALL_OF_FAME:1, TEXT_HALLOFFAME_OAK) is shown
  ...
  stepOutPendingRef.current = isDoorTile(mapId, mapInfo.tileset, landingTileId);
      ...
      // this — see pokeredChecklist.md's low-priority note; this is meant to be one of
...
// Route 22 Rival ambush (real OG: Route22DefaultScript checks player coords against
    ...
    // real OG has NO approach walk here at all (`movement: STAND` in gameData.json, no
    ...
    // ball has a real itemLocations.json entry). Real OG (scripts/FightingDojo.asm
    ...
    if (itemNpc) {
      ...
      if (!pickedUpRef.current.has(itemId)) {
        ...
        // itemLocations.json (extractedOgData) is keyed by map, with x/y in the OLD
        ...
        // match, rather than pre-converting this reference file like gameData.json was.
    ...
    // the regular staircase-style warp_events already in gameData.json (which alone already
    ...
    if (!battlesSuppressed && wildTable && onEncounterTile && Math.random() * 256 < wildTable.rate) {
      ...
      if (!repelled) {
        ...
        // extractedOgData/itemLocations.json has it at raw (50,4) == metatile (25,2), which
        // exactly matches gameData.json's ROCKET_HIDEOUT_B4F poke_ball NPC #8, so this already
...
function notifyPosition() {
  ...
  function checkLOS() {
    ...
    for (const npc of ms.mapInfo.npcs) {
      ...
      // npc.sight is already metatile-scale (OG-native, values 0/4 seen in gameData.json) —
  ...
  // brand-new "has Oak appeared" gameState flag that nothing else reads. `gameData.json`'s
  ...
  function objectText(mapId, tx, ty) {
    // gameData.json's bgEvents (OG-sourced, all 70 maps) take priority; OBJECT_TEXT
  ...
  // flat ANY_FACING flavor text. Deliberately kept OUT of gameData.json's bgEvents array (which
  ...
  // OG's own exact flag names (verified present in extractedOgData/eventFlags.json), so
  ...
  // port's gameData.json — some floors' door tiles decode as already-passable, others as
  ...
  // GATE6_UNLOCKED, already present verbatim in extractedOgData/eventFlags.json — reused via
  ...
  // as a normal sight/talk NPC via gameData.json's object_events regardless of quiz outcome in
  ...
  // generic itemLocations.json ground-item pipeline — no changes needed there.
  ...
  // ── Trainer battle dialogue (keyed by trainerClass from gameData.json NPC) ─
  ...
  // Real per-instance OG trainer quotes (extractedOgData/trainerText.json, 341 trainers
  ...
  // Real OG text for specific `scripted: true` npcDialogue.json entries whose sprite is
  ...
  // per-map OG text (extractedOgData/npcDialogue.json, keyed by mapId then 1-indexed
  ...
  // confirmed via gameData.json) can't be told apart by npcTrainerId alone, since it's purely
  ...
  function startDialogue(npc) {
    ...
    // order npcDialogue.json was keyed by (see npcText's comment).
    ...
    // the real, already-correctly-extracted rescue text from npcDialogue.json (npcText below)
    ...
    // generic npcDialogue.json extraction only captured the "before" branch (first CheckEvent
    ...
    // gameData.json exactly), each checking one specific badge via BadgeTextPointers — order
    ...
    // npcDialogue.json extraction only captured the opening line (no branch), same
    ...
    // branches on CheckEvent EVENT_RESCUED_MR_FUJI — the generic npcDialogue.json extraction
    ...
    // pattern as MR_FUJIS_HOUSE above — the generic npcDialogue.json extraction only captured
    ...
    // this port's gameData.json npcs arrays for every map below). TRADE_FOR_CHIKUCHIKU has no
    ...
    // npcDialogue.json's extraction only captured the "before" branch, same static-fallback
    ...
    // npcDialogue.json's extraction only captured the "don't have it yet" branch (matches
    ...
    // EVENT_BEAT_SABRINA; npcDialogue.json's extraction only captured the "before" branch, same
    ...
    // (SOFTBOILED) once. npcDialogue.json's real "5" text is only the pre-give line ("Here's a
    ...
    // npcDialogue.json text ("5") is the flattened give-text with no branch/grant logic, so
    ...
    // no shop dialogue in real OG at all — the npcDialogue.json "1" entry for this index is a
  ...
  useEffect(() => {
    const dn = e => {
      ...
      if (e.key === 'z' || e.key === 'Z') {
        ...
        // walkable, unlike visible poke_ball items which are stood on. hiddenItems.json uses
        // the same old raw-tile-doubled scale as itemLocations.json (confirmed: VIRIDIAN_FOREST
        ...
        // TV (see CLAUDE.md) — gameData.json's flat bgEvents entry (used by objectText() below)
  ...
  // ── Load gameData.json then Pallet Town ──────────────────────────────────
  useEffect(() => {
    ...
    fetch('/pokered/gameData.json')
      ...
      .then(async gd => {
        ...
        // Fall back to Pallet Town if the requested map doesn't exist in gameData.json
        if (gd.maps[startMap]) loadMap(startMap, sx, sy);
        else loadMap('PALLET_TOWN', 4, 9);
      })
      .catch(() => setLoadError('Could not load /pokered/gameData.json — run: node tools/extract-pokered.cjs'));
  }, [loadMap]);
