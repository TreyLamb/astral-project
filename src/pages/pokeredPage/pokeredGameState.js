...
import FISHING from './extractedOgData/fishing.json';
import EVENT_FLAG_LIST from './extractedOgData/eventFlags.json';
...
// pokemonData.json's `pokemon` dict and `learnsets` dict disagree on a handful of
...
// no equivalent field in pokemonData.json, so it's hand-entered here rather than guessed).
...
// generated once from gameData.json's actual npc placements (every real trainerClass-having
...
// pokemonData.json has no per-species growth-rate field to pick the right one from.
...
export const ITEM_EFFECTS = {
  ...
  HM06:          { category: 'hm06' },
  // Key items — never consumed. tier picks which extractedOgData/fishing.json table
...
// Stone evolutions — not present in pokemonData.json's learnsets (level-evos only),
...
// KADABRA/MACHOKE/GRAVELER/HAUNTER otherwise have empty `evos` arrays in pokemonData.json
...
// Source data: extractedOgData/eventFlags.json.
