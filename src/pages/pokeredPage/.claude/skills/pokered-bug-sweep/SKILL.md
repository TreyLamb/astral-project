...
> list. This is checkable purely from `.bst`/`.blk`/`gameData.json` — no
...
data in `extractedOgData/*.json` but zero references to that data anywhere
in the `.jsx` files."
...
- For a data-classification bug: write and run an actual script (Python/JS)
  ...
  `gameData.json`) that outputs every match. Do not manually eyeball a
...
- For a missing-wiring bug: grep every `.jsx` file for references to the
  ...
  the count on both sides (e.g. "14 sprites of type X exist in gameData.json
...
Then update `pokeredChecklist.md` per the normal checklist-sync workflow.
