...
- `{{SCOPE}}` = the exact list of map IDs in scope (e.g. `VIRIDIAN_CITY,
  ...
  `gameData.json`, not town names, since one town name usually covers several
  map IDs).
...
| `extractedOgData/itemLocations.json` | | visible ground items |
| `extractedOgData/hiddenItems.json` | | invisible/hidden items (Action-Button reveal) |
| `extractedOgData/marts.json` | | if this map has a clerk |
| `extractedOgData/npcDialogue.json` / `trainerText.json` | | |
| `extractedOgData/trainerAiTables.json` | | if gym/trainers present |
...
source files above, not from what's currently in `gameData.json` or the JS
...
- [ ] Dialogue text is real OG text (from `npcDialogue.json`/`trainerText.json`),
...
**Items**
- [ ] Every entry in `itemLocations.json` for this map is a real, pickable
      ground item at the right tile
- [ ] Every entry in `hiddenItems.json` for this map is present and requires
...
| VIRIDIAN_CITY | Hidden item | Full Heal behind sign | FAIL→FIXED | wasn't in gameData.json bgEvents; added from hiddenItems.json, tile (x,y) |
...
Update `pokeredChecklist.md` per the workflow rule in CLAUDE.md: move items
