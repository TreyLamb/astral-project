...
**Added 2026-08-06 — `cycleProjection.md` (sixth live doc).** Long-range projection of the cycle calendar to end-2027, in two variants (13-day and 11-day), with the pace chart mapped onto real dates. Built because the plan had only ever been dated two cycles out. It **changes nothing on its own** — it is analysis, not a new template — but it surfaced a hard arithmetic finding that supersedes the 14-cycle assumption below (see next block).

**⚠️ Finding 2026-08-06 — the 14-cycle framework does not fit the test date.** `pfra-2mile-training-plan.md`'s progression table (14 cycles, checkpoints at C3/C6/C9/C12, taper in C14) was written when the cycle was **11 days** — that doc says so itself. It was never re-checked after the move to 13 days. There are **127 days** from Cycle 3 Day 1 (Aug 27) to test day (Dec 31):
- **At 13 days: only 11 cycles fit.** Checkpoint Test 4 never happens, and Phase 3 collapses to ~2 cycles. The last checkpoint would be Nov 13 — six weeks of flying blind before the test.
- **At 11 days: 13 cycles fit**, all four checkpoints happen, and they land on weeks 4/9/14/18 versus the chart's 5/9/14/19 — near-exact, because the chart was built for 11-day cycles.
- Fitting all 14 cycles would need **10.6-day** cycles.
Do not plan against "14 cycles" without re-checking this. Full working, both projections, and the trade-off analysis live in `cycleProjection.md`.
...
**Corrected 2026-08-04 — `cycle.md`, `pfra-2mile-training-plan.md`, and `pfra-full-calendar.md` are NOT retired.** An earlier version of this file claimed they'd been merged into `training.md` and retired, but that was never actually followed through — all three have continued to be actively maintained doc-for-doc alongside `training.md` (verified directly: none are stubs/redirects, and this file's own "Cycle structure" section and "Instructions for future sessions" section both already treated `cycle.md` as the live source of the hard rules, contradicting the retirement claim above them). **Correct, current picture: the live doc set is all five files** — `training-context.md` (this file, decisions/history/rules, reads first, wins on conflict), `cycle.md` (canonical generic template + full session prescriptions — the ab/core circuit content lives here; the sled/carry hybrid did too until it was retired 2026-08-17), `training.md` (numbers-dense Cycle 1 & 2 dated tables), `pfra-full-calendar.md` (calendar view of the same dated cycles), `pfra-2mile-training-plan.md` (phase/pace/checkpoint framework). A stale "some docs are retired" claim being left uncorrected is exactly how the separate Lift A mess below happened (rule 1 was ambiguous in one doc and nobody caught it because doc liveness itself was unclear) — so this is fixed outright rather than re-flagged.

---

## Goal
- Event: **USAF PFRA 2-mile run** (was the ACFT 2-mile run — switched 2026-08-17, see the decision block at the bottom of this file; the run event itself is identical, so nothing structural changed).
- Target: **13:56** (6:58/mile), stretch goal **13:40** (6:50/mile).
...
Time trials on Aug 1–2 left significant soreness, so Cycle 1 opened with full rest + Lift A instead of a speed day, and a slow recovery run before Leg day. Full table lives in `pfra-full-calendar.md`. Known accepted deviation: Back day sits only 2 days from Speed 1 in this cycle specifically (not the standard 3) — a deliberate one-time call to avoid stretching the soft start further, not a new standing rule. Cycle 2 onward uses the standard 13-day template with no deviations.

## Decisions — Sled/Carry Hybrid Speed Day & Ab/Core Circuit (2026-08-04)

> **⚠️ The Sled/Carry Hybrid half of this section is HISTORICAL — the hybrid was retired 2026-08-17** with the switch to the USAF PFRA. Kept verbatim as the record of why it was built and placed where it was, so a future session doesn't rediscover the reasoning and assume it was lost. **Do not re-add it.** The Ab/Core Circuit half below is still live, with one amendment: both speed days now take the light add-on, not just Speed Day 2. See the ACFT → PFRA decision block at the bottom of this file.
...
Full session prescription (sled drag 4x25m, lateral sprint 4x50m, farmers carry 3x50m, then 2–4 classic 50m sprints, with loads/rest/progression) lived in `cycle.md` and was deleted 2026-08-17. Scaled to current level (2-mile ~18:00, Phase 1 strides-only) — deliberately far below the real ACFT Sprint-Drag-Carry standards (40lb KBs/hand, ~90lb+sled, 25m-each-way legs), which were the long-term reference point, not the Cycle 2 starting point. Those standards were recorded in the old `Guidelines_AFT`, which was deleted with the switch; `Guidelines_AF` holds the PFRA events and has no SDC content.
~~✂️ Open flag: the progression note in `cycle.md` mentions a possible Phase 3 full-event timed simulation...~~ **CLOSED 2026-08-17 — moot.** The flag asked whether the full 6-event ACFT was in scope. There is no ACFT in this plan anymore, and the PFRA has no equivalent event.
...
**Status 2026-08-06:** Trey asked to *see the numbers* for an 11-day cycle starting ~Cycle 3, explicitly to compare against 13 — **he has not yet said to switch.** That comparison is `cycleProjection.md`; the canonical template in `cycle.md` is untouched and still 13 days. If he does adopt 11, the two gaps being relaxed are **rule 2 (legs↔back) and rule 4 (back↔speed 2), both 3 days → 2**, with rule 3's Speed 1 → legs gap deliberately held at 3 (`cycle.md` already flags that one as possibly too short now that Speed Day 1 is the loaded sled/carry hybrid). Record that as a dated decision here when it happens — that record is exactly what this standing instruction exists to protect.
...
## Decision — ACFT → USAF PFRA (2026-08-17, Trey, direct)

**Trey is now prepping for the Air Force test, not the Army one.** The switch is smaller than it sounds, and the reason is worth stating up front so nobody re-opens work that doesn't need re-opening.

**The run event did not change.** The 2026 USAF PFRA uses a **2-mile run** — the same event the ACFT used. So the pace bands, 400m rep paces, the checkpoint schedule, the 3-phase framework, every hard rule in `cycle.md` except rule 12, the 13-day cycle derivation, and all of `cycleProjection.md`'s 13-vs-11-day analysis carry over **untouched**. This was a word-level edit pass over most of the doc set, not a rebuild.

**What the PFRA actually is:** a composite out of 100, taken twice a year (scored of record from 1 Jul 2026). Pass = 75 total **and** every component minimum; 90+ is Excellent. Four components:

| Component | Pts | Options | Trey's pick |
|---|---|---|---|
| Cardiorespiratory | 50 | 2-mile run **or** 20m HAMR shuttle | **2-mile run** |
| Body composition | 20 | Waist-to-height ratio | (only option) |
| Muscular strength | 15 | Push-ups 1 min **or** hand-release push-ups 2 min | **1-min push-ups** |
| Core endurance | 15 | Sit-ups 1 min **or** cross-leg reverse crunch **or** forearm plank | **undecided — sit-ups or plank** |

**Targets (Trey, direct — no scoring chart lookup needed):** 2-mile **13:56** · push-ups **56** · sit-ups **52**. Recorded in `Guidelines_AF`, which replaced `Guidelines_AF` (deleted).

**13:56 did not trigger a pace recalculation.** It is 6:58/mi against the old goal's 7:00/mi — 2 sec/mile. Every training pace in this plan is derived off the unchanged ~18:00 baseline, not off the goal, so the goal change is a cell edit in the goal rows and nothing more. The stretch goal stays 13:40.

**Rule 12 (Sled/Carry Hybrid) is retired.** It existed solely to train the ACFT's Sprint-Drag-Carry, which has no PFRA equivalent. **Speed Day 1 reverted to a classic speed session**, effective immediately (Cycle 2 D1, Fri 8/14, was the only cycle day that ever ran it as scheduled). The full prescription, the "why Day 1 not Day 10" analysis, and the Cycle 1 exception note were all removed from `cycle.md`. **The rule number was deliberately left occupied rather than renumbered** — rule 13 is cross-referenced *by number* from `training.md`, this file, and `training-with-pushups.md`, and renumbering would have silently broken all of them.

**Knock-on: rule 13 changed too.** Rule 13 denied Speed Day 1 any ab work, justified explicitly by "the loaded farmers carry already delivers an anti-rotation/anti-extension stimulus." With the carry gone that justification evaporates, so **both speed days now take the light ~5–8 min add-on**. Core endurance being a scored PFRA event makes leaving a speed day with zero core work harder to defend than it was. The robust 20-min circuit stays off D1 (it's the day before Lift A) — placement is still D2/D5/D9/D12, unchanged.

**Two open flags closed by this switch:**
- The ✂️ "is the full 6-event ACFT in scope?" flag (was in `cycle.md`'s hybrid progression and duplicated here) — **moot**, there is no such test anymore.
- The standing caveat that the D1→D4 gap into Lift B "was sized for a plain speed day, not a loaded hybrid" — **resolved**, D1 is a plain speed day again.

**Out of scope, by Trey's explicit instruction:** push-up and sit-up programming. He is handling those himself. Nothing in this doc set trains them, and `training-with-pushups.md`'s existing progression is unchanged. Do not build push-up or core *programs* here without being asked.

**✂️ Open — waist-to-height ratio.** 20 points, more than push-ups and core combined, and nothing in this plan addresses it. Not raised as a task; flagged so it isn't mistaken for an oversight.
...
- Once a real Checkpoint Test result exists, recalculate pace targets off that result (not the original ~18:00 estimate) for every cycle after it — this applies to `pfra-2mile-training-plan.md`'s pace chart and to speed/threshold prescriptions in the calendar.
- Extend `pfra-full-calendar.md` one cycle at a time as cycles actually happen, using the current version of `cycle.md` (which may have shortened) rather than assuming 13 days indefinitely.
