...
const TYPE_LABELS = {
  FILE: 'FILE type — Steady & reliable',
...
export default function GitmonHome() {
  ...
  if (phase === 'title') {
    return (
      <div className="gm-home">
        <div className="gm-title">GITMON<br />BLUE</div>
        <div style={{ fontSize: '3rem', margin: '4px 0' }}>🌐</div>
        <div className="gm-subtitle">VERSION 1.0</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, width: '100%', marginTop: 8 }}>
          {save && (
            <button className="gm-home-btn" onClick={handleContinue}>
              CONTINUE
            </button>
          )}
          <button className="gm-home-btn" onClick={handleNewGame}>
            NEW GAME
          </button>
        </div>
        <div className="gm-press-a">PRESS START</div>
      </div>
    );
  }
    ...
    return (
      <div className="gm-home">
        ...
        <form onSubmit={handleNameSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
          ...
          />
          <button className="gm-home-btn" type="submit">CONFIRM NAME</button>
        </form>
      </div>
    );
  }
    ...
    return (
      <div className="gm-home">
        ...
        <div style={{ display: 'flex', gap: 6, width: '100%' }}>
          <button className="gm-home-btn" style={{ flex: 1, opacity: 0.6 }} onClick={() => setPhase('name')}>
            BACK
          </button>
          <button
            className="gm-home-btn"
            style={{ flex: 1, opacity: selected ? 1 : 0.4 }}
            disabled={!selected}
