...
maps/*.blk: 225
Bash Inspect public/pokered assets and game_data.json structure
IN
cd "c:/Users/PCT/Projects/astral-project/public/pokered" && echo "=== public/pokered contents ===" && ls -la 2>/dev/null | head -40 && echo "" && echo "=== game_data.json top-level keys & size ===" && ls -la game_data.json 2>/dev/null && node -e "const g=require('./game_data.json'); console.log('top keys:', Object.keys(g)); if(g.maps){console.log('map count:', Array.isArray(g.maps)?g.maps.length:Object.keys(g.maps).length);}" 2>/dev/null || echo "node parse failed, trying grep"
...
-rw-r--r-- 1 TreyL 197609 500726 Jul 17 22:15 game_data.json
drwxr-xr-x 1 TreyL 197609      0 Jun 25 19:51 maps
-rw-r--r-- 1 TreyL 197609 128486 Jun 26 00:57 pokemon_data.json
drwxr-xr-x 1 TreyL 197609      0 Jun 26 00:13 sprites
drwxr-xr-x 1 TreyL 197609      0 Jun 25 19:51 tilesets

=== game_data.json top-level keys & size ===
-rw-r--r-- 1 TreyL 197609 500726 Jul 17 22:15 game_data.json
...
Files to inspect:
- `c:/Users/PCT/Projects/astral-project/public/pokered/game_data.json` — top keys are `maps`, `collision`, `grassTiles`, `ledges`. `maps` is an object keyed by MAP_ID. Each map likely has `warps` (array), `npcs` (array), and possibly `bgEvents`, `signs`, etc.
- `c:/Users/PCT/Projects/astral-project/src/pages/pokered_page/extracted_og_data/*.json` — several JSON files.

Compute and report:

1. **game_data.json map structure**: Load it with node. Report: total number of maps. Then pick 3 sample maps (e.g. PALLET_TOWN, VIRIDIAN_CITY, CERULEAN_CITY) and print the full JSON of ONE small map so I can see the exact per-map schema (what fields exist on the map, on each warp, on each npc).
...
and kills the need for hacks like the Trashed House stopgap. Wire the existing `trainer_text.json`
...
**0e. Map-count reconciliation.** Identify the 2 OG maps absent from `game_data.json` (223 vs 221)
...
Region phases also do their share of: NPC dialogue (from `npc_dialogue.json`/`trainer_text.json`/
...
2. The full JSON array (write it to `C:/Users/PCT/AppData/Local/Temp/claude/c--Users-PCT-Projects-astral-project-src-pages-pokered-page/3a0502f3-9d31-4736-ab93-5aef43766e9b/scratchpad/event_flags.json` using a bash heredoc/redirect so it's saved — do NOT use an editor tool, just shell redirect).
...
Read C:\Users\PCT\AppData\Local\Temp\claude\c--Users-PCT-Projects-astral-project-src-pages-pokered-page\3a0502f3-9d31-4736-ab93-5aef43766e9b\scratchpad\event_flags.json
...
Phase 0e: reconcile the 2 OG maps missing from game_data.json (223 vs 221)
...
Write c:\Users\PCT\Projects\astral-project\src\pages\pokered_page\extracted_og_data\event_flags.json
2 lines
...
Now verify the registry count and run the 0e map reconciliation (comparing OG's canonical map-constants list against game_data.json keys):

Bash Verify flag count and reconcile OG maps vs game_data.json
IN
cd "c:/Users/PCT/Projects/astral-project" && node -e '
const flags = require("./src/pages/pokered_page/extracted_og_data/event_flags.json");
console.log("event_flags.json entries:", flags.length);
...
node -e '
const fs=require("fs");
const g=require("./public/pokered/game_data.json");
...
UNUSED_MAP_F3
UNUSED_MAP_F4
--- In game_data.json but NOT in OG map_const ---
(none)
...
cd "c:/Users/PCT/Projects/astral-project" && node -e '
const g=require("./public/pokered/game_data.json");
console.log("UNDERGROUND* maps in game_data.json:");
...
OUT
UNDERGROUND* maps in game_data.json:
...
Phase 0e: reconcile OG maps missing from game_data.json
...
0a	507 OG event flags → extracted_og_data/event_flags.json	✅ Done	node count = 507 distinct
...
UNDERGROUND_PATH_ROUTE_7 is genuinely missing from game_data.json — Route 7 warps into it, so that warp is currently broken. The other 26 absent maps are confirmed junk (22 UNUSED_MAP_* + 4 _COPY). Map coverage is otherwise complete. → convert in Phase 3.
...
=== all distinct growth-rate constants used ===
     ...
     26 GROWTH_SLOW
Four curves confirmed (FAST×5, MEDIUM_FAST×80, MEDIUM_SLOW×40, SLOW×26 = 151). Let me extract the species→growth-rate map and write it as project data, matching it to pokemon_data.json's species keys:
...
const pdata=require("./public/pokered/pokemon_data.json");
...
fs.writeFileSync("src/pages/pokered_page/extracted_og_data/growth_rates.json", JSON.stringify(out,null,0)+"\n");
...
console.log("\n=== DANGLING DEST (dest map not in game_data.json):",danglingDest.length,"===");
...
=== DANGLING DEST (dest map not in game_data.json): 4 ===
...
for(const id of cluster){
  const m=g.maps[id];
  if(!m){console.log(id+": (NOT IN game_data.json)");continue;}
...
3. **From these two JSON files** (`c:/Users/PCT/Projects/astral-project/src/pages/pokered_page/extracted_og_data/item_locations.json` and `hidden_items.json`): list any entries keyed by those 5 map IDs.
...
node -e 'const g=require("../../../public/pokered/game_data.json"); console.log(JSON.stringify(g.maps.ROUTE_1.wild));'
...
0a Event-flag system: 507 OG flags → event_flags.json, wired into pokeredGameState.js as a validated registry (typo-guard on setEvent/hasEvent/clearEvent). vite build passes. Discovered the existing event helpers were dead code (zero callers) — so this is the vocabulary the region phases will actually wire.
...
node -e 'const g=require("./public/pokered/game_data.json");console.log(Object.keys(g.maps).filter(k=>k.includes("ROUTE_2")||k.includes("DIGLETT")||k.includes("OAKS_AIDE")||k.includes("ROUTE2")).join("\n"))'
...
IN
cd "c:/Users/PCT/Projects/astral-project" && echo "=== port ROUTE_2_GATE npcs ===" && node -e 'const g=require("./public/pokered/game_data.json");const m=g.maps.ROUTE_2_GATE;console.log("warps:",JSON.stringify(m.warps));console.log("npcs:",JSON.stringify(m.npcs,null,1))'
echo ""
...
0a Event-flag system: 507 OG flags → event_flags.json, wired into pokeredGameState.js as a typo-guarded registry. Found the existing helpers were dead code (zero callers). ✅ build-verified.
...
IN
cd "c:/Users/PCT/Projects/astral-project" && git add "src/pages/pokered_page/extracted_og_data/event_flags.json" "src/pages/pokered_page/extracted_og_data/growth_rates.json" "src/pages/pokered_page/pokeredGameState.js" "src/pages/pokered_page/PokeredOverworld.jsx" "pokemon_OG/bugtracking/Lastmap markdowns/POKERED_CHECKLIST.md" "pokemon_OG/bugtracking/Lastmap markdowns/BATTLE_MECHANICS_CHANGE_PROPOSALS.md" "asvab_master_study_guide.md" && echo "=== staged ===" && git status --short | grep -E "^[AMR]" && echo "=== committing ===" && git commit -q -m "$(cat <<'EOF'
pokered: Phase 0 foundation + Phase 1 audit (100%-completion roadmap)

- Event-flag system (0a): 507 OG flags -> event_flags.json, typo-guarded
...
- Battle audit (0c): confirmed flinch/confusion/crit already implemented;
  ...
  changed). growth_rates.json extracted (unwired, pending approval).
...
warning: in the working copy of 'src/pages/pokered_page/extracted_og_data/event_flags.json', LF will be replaced by CRLF the next time Git touches it
warning: in the working copy of 'src/pages/pokered_page/extracted_og_data/growth_rates.json', LF will be replaced by CRLF the next time Git touches it
=== staged ===
R  "pokemon_OG/bugtracking/Lastmap markdowns/asvab_master_study_guide.md" -> asvab_master_study_guide.md
...
A  src/pages/pokered_page/extracted_og_data/event_flags.json
A  src/pages/pokered_page/extracted_og_data/growth_rates.json
...
FOUNDATION: `pokeredGameState.js` has an event-flag system: `EVENT_FLAGS` (Set of 507 OG flag names from extracted_og_data/event_flags.json), `hasEvent(state,name)`, `setEvent(state,name)`→newState, `clearEvent`. Use these real OG flag names for NEW story gates instead of ad-hoc booleans.
...
KEY FILES: PokeredOverworld.jsx, PokeredApp.jsx, pokeredGameState.js, public/pokered/game_data.json (maps/warps/npcs — under public/, NOT pokered_page), extracted_og_data/*.json (npc_dialogue.json, trainer_text.json, item_locations.json, hidden_items.json).
...
R5. FULL DIALOGUE + ITEM AUDIT — for every NPC and every ground/hidden item across the scope maps, check the port against OG (extracted_og_data/npc_dialogue.json, trainer_text.json, item_locations.json, hidden_items.json, and the scripts/text). Wire any NPC currently showing generic "..." / sprite-fallback, and any missing item, to real OG content. Report a per-map PASS/FIXED list.
...
KEY FILES: PokeredOverworld.jsx, PokeredApp.jsx, PokeredBattle.jsx (battle UI/flow), battleEngine.js (DON'T touch mechanics), pokeredGameState.js, public/pokered/game_data.json (maps/warps/npcs, under public/), extracted_og_data/*.json.
...
R4. SNORLAX (ROUTE_12 and ROUTE_16) — each route has a SNORLAX-sprite NPC blocking the path. FIRST verify these Snorlax NPCs exist in game_data.json for ROUTE_12 and ROUTE_16 (check OG object_events; ADD them to game_data.json if missing). Using the POKE_FLUTE near/facing Snorlax wakes it → forced wild battle (level 30). Wire: EVENT_FIGHT_ROUTE12_SNORLAX/ROUTE16 on flute use, EVENT_BEAT_ROUTE12_SNORLAX/ROUTE16 after the battle; after beaten/caught, Snorlax despawns and the path is passable. Trace scripts/Route12.asm + scripts/Route16.asm + how the port triggers a forced wild battle (see the Old Man tutorial `oldManDemo` forced-encounter path in PokeredOverworld.jsx/PokeredBattle.jsx as a mechanism reference).
