...
function effStat(mon, key) {
  let v = mon[key] * stageMult(mon.stages?.[key]);
...
function computeDamage(attacker, defender, move, moveName, pokemonData, { crit = false, explode = false } = {}) {
  ...
  // Gen 1 crits ignore ALL stat modifiers, screens and burn on both sides.
  const atkStat = crit ? attacker[special ? 'spc' : 'atk'] : effStat(attacker, special ? 'spc' : 'atk');
  let defStat = crit ? defender[special ? 'spc' : 'def'] : effStat(defender, special ? 'spc' : 'def');
  if (!crit) {
...
export function pickEnemyMove(mon, pokemonData) {
...
export function performRound(player, enemy, playerAction, pokemonData, { isTrainerBattle = false, enemyMoveName = null } = {}) {
  const msgs = [];
  ...
  const eMoveName = enemyMoveName ?? (isLocked(enemy) ? null : pickEnemyMove(enemy, pokemonData));
