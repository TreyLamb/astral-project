...
## ⚠️ PART 28 — the "IN-PROGRESS HANDOFF" below was SUPERSEDED same-day, read this instead

A second, independent session picked up PART 28 in parallel with the one that wrote the
in-progress handoff below (both sessions started from the same `[L]`-unlocked board state and
neither could see the other's uncommitted work). That second session finished a **complete,
independently-verified** implementation — different internal structure (`engine/exam.js`'s
`EXAM_PLAN`/`assembleDrill`-based sequencing rather than `EXAM_STEPS`/`buildExamQuestions`; a
separate `ExamConfig.jsx` entry screen plus `ExamRunner.jsx` rather than one three-phase
component) — verified end-to-end with all QC gates green AND a real full click-through in a
browser (see the "2026-08-26 — PART 28: full-length Form T exam runner" entry further down this
file, and `docs/afoqt/HANDOFF.md`'s "PART 28 design record" for the complete write-up, including
a real React state/localStorage bug that session found and fixed).

**The TR zero-questions bug described below is specific to the discarded `buildExamQuestions`
implementation and does not exist in the version that shipped** — the shipped version calls
`assembleDrill` directly (the same, already-proven code path `DrillConfig`'s own "Full subtest"
button already exercises for every subtest, TR included), rather than re-implementing that logic
in a new function. A real full exam run (all 11 scored subtests, ~310 questions) completed with
Table Reading answering all 40 questions correctly every time it was run.

The section below is kept **as a historical record of the parallel session's own work and
reasoning**, not as a live task list — do not act on its "Not yet done" checklist; PART 28 is
already `[x]` on the board.
...
*(Historical - not applicable. The other session's complete, verified implementation shipped
instead; see the superseded-note above. Left unedited below as the record of this session's own
plan at the time it paused.)*
