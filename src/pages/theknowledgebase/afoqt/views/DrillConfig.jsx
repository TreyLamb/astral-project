...
export default function DrillConfig() {
  ...
  const start = () => {
    ...
    navigate(`/TKB/afoqt/drill/run?subtest=${subtest}&count=${count}&mode=${mode}&pressure=${pressure}`);
  };
        ...
        <div className="afq-row afq-wrap-row">
          ...
          {meta && (
            <button
              ...
              onClick={() => { setCount(meta.questions); setMode('exam'); setPressure(1); }}
      ...
      <section>
        ...
        <div className="afq-row">
          {['untimed', 'paced', 'exam'].map((m) => (
            <button key={m} className={'afq-btn' + (mode === m ? ' afq-primary' : '')} onClick={() => setMode(m)}>{m}</button>
          ))}
        </div>
        {mode !== 'untimed' && (
          <>
